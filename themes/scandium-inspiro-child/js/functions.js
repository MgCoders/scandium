...
(function ($) {
    ...
    $(function () {
        ...
        if (typeof wpz_currPage != 'undefined' && wpz_currPage < wpz_maxPages) {
            ...
            //EDITADO PARA QUE SEA MULTIIDIOMA!
            var load_txt = "Cargar Más";
            ...
            $('.navigation').empty().append('<a class="btn btn-primary" id="load-more" href="#">'+load_txt+'&hellip;</a>');

            $('#load-more').on('click', function () {
                if (wpz_currPage < wpz_maxPages) {
                    $(this).text(loading_txt+'...');
                    wpz_currPage++;

                    $.get(wpz_pagingURL + wpz_currPage + '/', function (data) {
                        //console.log(data);
                        var $newItems = $('.portfolio-grid a', data).addClass('hidden').hide();

                        $folioitems.append($newItems);
                        $folioitems.find('a.hidden').fadeIn().removeClass('hidden');

                        if ((wpz_currPage + 1) <= wpz_maxPages) {
                            $('#load-more').text(load_txt+'\u2026');
                        } else {
